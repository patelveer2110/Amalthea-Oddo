...
export const tasksApi = {
  ...
  create: (projectId: string, data: Partial<Task>) => client.post<Task>(`/api/v1/projects/${projectId}/tasks`, data),
  update: (id: string, data: Partial<Task>) => client.put<Task>(`/api/v1/tasks/${id}`, data),
  move: (id: string, state: Task["state"]) => client.post<Task>(`/api/v1/tasks/${id}/move`, { state }),
}
